  ...
  <div class="index">
    ...
    <div class="divMain">
         ...
         <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <span v-for="(item,index) in scope.row.users" :key="item.id" v-if="index>1">
                {{index>2&&(scope.row.users.length-1)>2?item.realName+"...":item.realName+","}}
              </span>
              <span v-else>
                {{item.realName}}
              </span>
        ...
        <template slot-scope="scope">
          <el-button
            size="mini"
           > <router-link :to="{path:'/dashboard/detail',query:{id:scope.row.id}}">查看</router-link> </el-button>
          <el-button
            type="primary"
            @click='outExe(scope.row)'
           > 下载 </el-button>
